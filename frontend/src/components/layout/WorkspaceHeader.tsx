...
interface WorkspaceHeaderProps {
	...
	autoProveEnabled?: boolean;
	onAutoProveToggle?: (enabled: boolean) => void;
	onRunAgents?: () => void;
	showControls?: boolean;
}

export function WorkspaceHeader({
	breadcrumbs = [{ label: "Workspaces" }, { label: "Riemann Zeroes" }],
	status = "draft",
	autoProveEnabled = false,
	onAutoProveToggle,
	onRunAgents,
	showControls = true,
				...
				{showControls && (
					<>
						...
						<div className="flex items-center gap-2 mr-4">
							...
							<button
								onClick={() => onAutoProveToggle?.(!autoProveEnabled)}
								className={`relative w-8 h-4 rounded-full transition-colors ${autoProveEnabled ? "bg-[var(--text-primary)]" : "bg-[var(--border-secondary)]"
									}`}
							>
								<span
									className={`absolute top-0.5 w-3 h-3 bg-[var(--bg-primary)] rounded-full shadow transition-all ${autoProveEnabled ? "left-[18px]" : "left-0.5"
										}`}
								/>
							</button>
						</div>
						...
						{/* Run Agents button */}
						<button onClick={onRunAgents} className="btn btn-primary">
