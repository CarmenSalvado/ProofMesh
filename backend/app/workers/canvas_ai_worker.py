...
def normalize_library_item_kind(raw_kind: str | None):
    """Map arbitrary node kinds to a valid LibraryItemKind enum value."""
    from app.models.library_item import LibraryItemKind

    kind_text = (raw_kind or "CLAIM").strip().upper()
    try:
        return LibraryItemKind(kind_text)
    except ValueError:
        return LibraryItemKind.NOTE


def infer_generated_node_type(content: str) -> str:
    """Infer a reasonable node type from free-form model text."""
    lower = content.lower()
    if "formal test" in lower or "lean test" in lower or "lean theorem" in lower:
        return "FORMAL_TEST"
    ...
    if "lemma" in lower:
        return "LEMMA"
    if "claim" in lower or "proposition" in lower:
        return "CLAIM"
    if "counterexample" in lower:
        return "COUNTEREXAMPLE"
    if "computation" in lower or "calculation" in lower:
        return "COMPUTATION"
    if "resource" in lower or "reference" in lower:
        return "RESOURCE"
    if "idea" in lower or "approach" in lower:
        return "IDEA"
    if "content" in lower:
        return "CONTENT"
    return "NOTE"
...
def summarize_node_title(content: str, fallback: str) -> str:
    """Create a compact title from text content."""
    ...
    if not title:
        return fallback
    if len(words) >= 8 or len(first_sentence) > len(title):
        return f"{title}..."
    return title
...
def normalize_generated_node_payload(node: dict, fallback_content: str = "") -> dict:
    """
    Normalize AI node payload so each kind has coherent fields for the canvas model.
    """
    kind = normalize_library_item_kind(node.get("type"))
    raw_content = str(node.get("content") or fallback_content or "").strip()
    formula = node.get("formula")
    lean_code = node.get("lean_code") or node.get("leanCode")

    if kind.value == "COMPUTATION":
        python_code = (
            node.get("python_code")
            or extract_fenced_code(raw_content, "python")
            or extract_fenced_code(raw_content, "py")
            or raw_content
        ).strip()
        if not python_code:
            python_code = "print('computation node')"
        return {
            "kind": kind,
            "content": python_code,
            "formula": None,
    ...
    if kind.value == "FORMAL_TEST":
        resolved_lean = (
            str(lean_code).strip() if lean_code else extract_fenced_code(raw_content, "lean")
        )
        summary = raw_content or "Formal Lean test generated by AI."
        return {
            "kind": kind,
            "content": summary,
            "formula": formula,
            "lean_code": resolved_lean or None,
        }
    ...
    return {
        ...
        "content": raw_content,
        "formula": formula,
        "lean_code": str(lean_code).strip() if lean_code else None,
    }


def build_fallback_diagram(response: str, trace, prompt: str | None = None) -> dict:
    """
    Build a minimal node/edge diagram when the model does not return JSON.
    This prevents empty runs with `created_nodes = 0`.
    """
    import re
    ...
    if not candidates and trace and getattr(trace, "steps", None):
        for step in trace.steps:
            step_text = (getattr(step, "content", "") or "").strip()
            if len(step_text) < 20:
                continue
    ...
    if not candidates and prompt:
        ...
        if prompt_text:
            candidates.append(f"Exploration goal: {prompt_text}")

    nodes = [
        {
            "id": f"auto_{index + 1}",
            "type": infer_generated_node_type(text),
            "title": summarize_node_title(text, f"Insight {index + 1}"),
            "content": text,
        }
        for index, text in enumerate(candidates)
    ]
    edges = [
        {"from": nodes[index - 1]["id"], "to": nodes[index]["id"], "type": "implies"}
        for index in range(1, len(nodes))
    ]
    return {"nodes": nodes, "edges": edges}


async def create_library_item_from_node(
    db: AsyncSession,
    problem_id: UUID,
    run_id: UUID,
    node: dict,
    ...
    """Persist a generated canvas node as a library item and return it."""
    from app.models.library_item import LibraryItem, LibraryItemStatus

    normalized = normalize_generated_node_payload(node, fallback_content=fallback_content)
    ...
    item = LibraryItem(
        ...
        lean_code=normalized["lean_code"],
        status=LibraryItemStatus.PROPOSED,
        authors=[{"type": "agent", "id": "orchestrator", "name": "Canvas AI"}],
        source={"agent_run_id": str(run_id)},
        dependencies=[],
    )
    db.add(item)
    await db.flush()
...
async def process_explore_streaming(
    ...
    try:
        ...
        # Fallback: derive nodes from plain response / reasoning trace
        if not nodes:
            fallback = build_fallback_diagram(response, trace, prompt=prompt)
        ...
        for node in nodes:
            ...
            library_item = await create_library_item_from_node(
                db=db,
            ...
            await publish_event(redis_client, problem_id, {
                ...
                "node": {
                    **node,
                    "id": str(library_item.id),
                    "temp_id": temp_id,
                    "type": library_item.kind.value,
                },
            })

        deps_by_target: dict[UUID, set[UUID]] = {}
        for edge in edges:
            from_id = node_id_map.get(str(edge.get("from", "")))
            to_id = node_id_map.get(str(edge.get("to", "")))
            if not from_id or not to_id:
                continue
            if to_id not in deps_by_target:
                deps_by_target[to_id] = set()
            deps_by_target[to_id].add(from_id)
        ...
        if deps_by_target:
            ...
            result = await db.execute(
                select(LibraryItem).where(LibraryItem.id.in_(list(deps_by_target.keys())))
            )
            for item in result.scalars().all():
                deps = deps_by_target.get(item.id)
                if deps:
                    item.dependencies = sorted(deps, key=str)
            await db.commit()
        ...
        for i, proposal in enumerate(proposals):
            ...
            if isinstance(diagram, dict):
                ...
                for node in nodes:
                    ...
                    fallback_content = getattr(proposal, "content", "") or ""
                    library_item = await create_library_item_from_node(
                        db=db,
                        problem_id=UUID(problem_id),
                        run_id=run_id,
